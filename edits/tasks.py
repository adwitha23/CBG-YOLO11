...
from ultralytics.nn.modules.block import CBAM, BiFPN, CbamBiFPN_Neck, SPDConv
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
        ...
        elif m in {BiFPN, CbamBiFPN_Neck}:
            # Extract base channels
            c2 = args[0]
            # Apply width scaling and hardware rounding
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            # Extract backbone tap-off channels dynamically
            c1_list = [ch[x] for x in f]
            
            # Reconstruct args: c1_list, scaled out_channels, and any trailing args (like num_fusions)
            args = [c1_list, c2, *args[1:]]
            
            # CRITICAL: Force c2 to be an integer (make_divisible sometimes returns float)
            # This ensures the Detect head registers the correct input shape
            c2 = int(c2)
        elif m in frozenset(
            {
                Detect,
            ...
            args.extend([reg_max, end2end, ([ch[x] for x in f] if isinstance(f, list) else [ch[f], ch[f], ch[f]])])
