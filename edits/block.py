...
# Paste this at the bottom of block.py
class SPDConv(nn.Module):
    """Space-to-Depth Convolution for preserving high-resolution, low-contrast features."""
    def __init__(self, c1, c2, k=3, s=1):
        super().__init__()
        # c1 * 4 because stacking 4 sub-maps quadruples the input channels
        self.conv = Conv(c1 * 4, c2, k, s)

    def forward(self, x):
        # Slice and stack the spatial dimensions into the channel dimension
        x = torch.cat([
            x[..., ::2, ::2],    # Top-left
            x[..., 1::2, ::2],   # Bottom-left
            x[..., ::2, 1::2],   # Top-right
...
class BiFPN(nn.Module):
    def __init__(self, in_channels_list, out_channels):
        ...
        self.w = nn.Parameter(torch.ones(2, 3), requires_grad=True)
        self.epsilon = 1e-4

    def forward(self, feats):
        p3, p4, p5 = [proj(f) for f, proj in zip(feats, self.proj)]
        w1 = torch.relu(self.w[0])
        w1_norm = w1 / (torch.sum(w1, dim=0) + self.epsilon)
        p4_td = w1_norm[0] * p4 + w1_norm[1] * F.interpolate(p5, size=p4.shape[-2:], mode='nearest')
        
        w2 = torch.relu(self.w[1])
        w2_norm = w2 / (torch.sum(w2, dim=0) + self.epsilon)
        p3_out = p3 
        p4_out = w2_norm[0] * p4_td + w2_norm[1] * F.max_pool2d(p3_out, kernel_size=2)
        p5_out = w2_norm[2] * p5 + w2_norm[1] * F.max_pool2d(p4_out, kernel_size=2)
        return [p3_out, p4_out, p5_out]


class BiFPN_Layer(nn.Module):
    """
    A single iteration of the BiFPN Top-Down and Bottom-Up pathways using 
    Fast Normalized Fusion logic.
    """
    def __init__(self, channels, epsilon=1e-4):
        super().__init__()
        self.epsilon = epsilon

        # 1. Learnable weights for Fast Normalized Fusion
        # w1: 2 inputs for P4_td (P4_in, P5_in)
        self.w1 = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True) 
        # w2: 3 inputs for P4_out (P4_in, P4_td, P3_out)
        self.w2 = nn.Parameter(torch.ones(3, dtype=torch.float32), requires_grad=True) 
        # w3: 2 inputs for P5_out (P5_in, P4_out)
        self.w3 = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True) 

        # 2. Convolutions for feature refinement after fusion
        # Using depthwise separable convolutions here is standard for BiFPN to keep FLOPS low
        self.conv_p4_td = nn.Conv2d(channels, channels, 3, padding=1, groups=channels)
        self.conv_p3_out = nn.Conv2d(channels, channels, 3, padding=1, groups=channels)
        self.conv_p4_out = nn.Conv2d(channels, channels, 3, padding=1, groups=channels)
        self.conv_p5_out = nn.Conv2d(channels, channels, 3, padding=1, groups=channels)
    ...
    def forward(self, x):
        ...
        # Normalize weights: ensure they are positive and sum to 1
        w1 = torch.relu(self.w1) / (torch.sum(torch.relu(self.w1), dim=0) + self.epsilon)
        w2 = torch.relu(self.w2) / (torch.sum(torch.relu(self.w2), dim=0) + self.epsilon)
        w3 = torch.relu(self.w3) / (torch.sum(torch.relu(self.w3), dim=0) + self.epsilon)

        # --- Top-Down Pathway ---
        p4_td = self.conv_p4_td(self.act(w1[0] * p4_in + w1[1] * self.upsample(p5_in)))
        
        # P3 acts as the turnaround wall (Standard addition is sufficient here)
        p3_out = self.conv_p3_out(self.act(p3_in + self.upsample(p4_td)))

        # --- Bottom-Up Pathway ---
        p4_out = self.conv_p4_out(self.act(w2[0] * p4_in + w2[1] * p4_td + w2[2] * self.downsample(p3_out)))
        p5_out = self.conv_p5_out(self.act(w3[0] * p5_in + w3[1] * self.downsample(p4_out)))

        return [p3_out, p4_out, p5_out]


class CbamBiFPN_Neck(nn.Module):
    """
    The wrapper module that applies CBAM once, aligns channels, 
    and then loops the BiFPN layer.
    """
    def __init__(self, c1_list, out_channels=256, num_fusions=3):
        super().__init__()
        
        # 1. Apply CBAM BEFORE the loop and channel reduction
        self.cbam_p3 = CBAM(c1_list[0])
        self.cbam_p4 = CBAM(c1_list[1])
        self.cbam_p5 = CBAM(c1_list[2])

        # 2. Align channels from backbone depths to out_channels
        self.align_p3 = nn.Conv2d(c1_list[0], out_channels, 1)
        self.align_p4 = nn.Conv2d(c1_list[1], out_channels, 1)
        self.align_p5 = nn.Conv2d(c1_list[2], out_channels, 1)

        # 3. Initialize the multiple fusion layers
        self.bifpn_layers = nn.ModuleList([
            BiFPN_Layer(channels=out_channels) for _ in range(num_fusions)
        ])

    def forward(self, x):
        p3, p4, p5 = x

        # Execute CBAM Spatial & Channel Attention
        p3 = self.cbam_p3(p3)
        p4 = self.cbam_p4(p4)
        p5 = self.cbam_p5(p5)
        ...
        # Iteratively pass the list of tensors through the BiFPN blocks
        features = [p3, p4, p5]
